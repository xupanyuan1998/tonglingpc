<template>
  <div class="hello">
    ...
    <div class="container">
      <div class="nav">
        <span @click="goindex">首页</span><b> ></b><span >政策信息</span>
      </div>
      <div class="line"></div>
    </div>
    <div class="warp">
      <div class="left">
        <h3 v-for="(item,idx) in leftlist" :class="leftShow==idx?'select':''" @click="cleft(idx,item)">{{item.affairsName}}</h3>
      </div>
      <div class="right">
        <div class="serch">
...
<!--          <router-link tag="a" to="/information/informationMore/informationdetali">本市召开中小企业高质量发展座谈会23个优质中小企业重点项目集中开工</router-link>-->
          <li v-for="(item,idx) in newsList" :key="idx" :class="idx%5==4?'five':''"><b></b><router-link tag="a" :to="{path:'/information/informationMore/informationdetali',query:{id:item.policyId,leftId:leftShow}}">{{item.title}}</router-link><span>{{clearFen(item.publishTime)}}</span></li>
        </ul>
        <div class="page">
          <!--        分页组件-->
          <div>
...
import '../../../assets/css/page.less'
    ...
    export default {
        ...
        methods:{
            ...
            cleft(i,b){
              this. leftShow=i;
              this.categoryid=b.affairsId;
              this.rightname=b.affairsName;
                this.getnewList(20,1,this. categoryid,this.serchs);
            },
            prePage(){
                this.currentPage -= 1;
        ...
        created() {
          ...
          if(id==undefined){
            ...
            this.axios.post('/web/policy/policycategory').then((data)=>{
              ...
              this.rightname=data.data.data[0].affairsName;
              this.categoryid=data.data.data[0].affairsId;
              this.getnewList(20,1,this. categoryid,this.serchs);
            });
          }else{
            this.leftShow=id;
            this.axios.post('/web/policy/policycategory').then((data)=>{
              this. leftlist=data.data.data.reverse();
              this.rightname=data.data.data[id].affairsName;
              this.categoryid=data.data.data[id].affairsId;
              this.getnewList(20,1,this. categoryid,this.serchs);
            });
          }

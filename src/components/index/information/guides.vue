<template>
  <div class="hello">
    ...
    <div class="container">
      <div class="nav">
        <span @click="goindex">首页</span><b> ></b><span >办事指南</span>
      </div>
      <div class="line"></div>
    </div>
    <div class="warp">
      <div class="left">
        <ul>
          ...
          <li v-for="(item,idx) in leftList" :key="idx" :class="leftSelect==item.deptId?'slec':''" @click="leftS(item.deptId,item.deptName)"><span >{{item.deptName}}</span></li>
        </ul>
      </div>
      <div class="right">
        <div class="serch">
        ...
        <h1>{{samlltitle==''?'全部':samlltitle}}</h1>
        <ul class="addlist" v-if=" newsList.length>1">
          <li v-for="(item,idx) in newsList" :key="idx" :class="idx%5==4?'five':''"><em>[{{item.departmentName}}]</em><router-link tag="a" :to="{path:'/information/guides/guidesDetali',query:{id:item.guideId}}">{{item.title}}</router-link> <span>{{clearFen(item.publishTime)}}</span></li>
        </ul>
        <div class="liep">
          暂无内容
        </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getnewList(a,b,c,d){
                this.axios.post('/web/guide/list',{title:d,departmentId:c,size:a,current:b,userId:this.userId}).then(({data})=>{
            ...
            //添加收藏
            addshou(i){
                var a='#'+this.$route.fullPath+'/guidesDetali?id='+i.guideId;
                console.log(a);
                if(this.msg==null){
                    this.placeshow=true;
                }else{
                    this.axios.post('/web/usercollection/usercollection',{contentId:i.guideId,state:1,type:3,userId:this.msg.userId,Url:a}).then(({data})=>{
                        if(data.code==10001){
            ...
            //取消收藏
            cancleshou(i){
                this.axios.post('/web/usercollection/usercollection',{contentId:i.guideId,state:2,type:3,userId:this.msg.userId,Url:' '}).then(({data})=>{
                    if(data.code==10001){

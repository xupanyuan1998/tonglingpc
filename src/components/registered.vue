<template>
  <div class="hello">
   ...
   <div class="con">
     <div class="res">
       <img src="../../static/images/116.png" alt=""><span>实名注册</span>
       <b>注:以下带*的为必填项</b>
     </div>
     <div class="warp">
       <ul>
         <li>
           <p><b>*</b><span>手机号码:</span></p>
           <div>
         ...
         <li>
           <p><b>*</b><span>姓名:</span></p>
           <div>
             <input type="text" v-model="user" @input.stop="getinput" placeholder="真实姓名">
             <em id="user">姓名不能为空</em>
           </div>
         </li>
         <li>
           <p><b>*</b><span>身份证号码:</span></p>
           <div>
             <input type="text" v-model="cardNum" @input.stop="yancard" placeholder="请输入身份证号">
             <em id="cardNum">身份证号码格式不正确</em>
           </div>
         </li>
         <li>
           <p><b>*</b><span>密码:</span></p>
           <div>
             <div class="passw">
               <input :type="passes" v-model="password" @focus="changeborder" @blur="changes" @input.stop="yanpassword" placeholder="请输入6-16为数字、字符或符号">
         ...
         <li class="yan">
           <p><b>*</b><span>手机验证码:</span></p>
           <div>
             <input type="text" v-model="msgYan" placeholder="请输入验证码">
             <span v-if="isdao=='false'" @click.stop="getMsgnum">获取短信验证码</span><span v-if="isdao=='true'">{{dao}}</span>
         ...
         <li>
           <p></p>
           <div class="xieyi">
           <strong v-if="agree" @click="faagree"></strong><i v-if="!agree" @click="tragree"></i><span>我已阅读并同意 <a href="javascript:">《铜陵市民营经济云服务智慧平台注册协议》</a></span>
         </div>
         </li>
       </ul>
          ...
          changes(){
           ...
           getMsgnum(){
               ...
               if(!phonereg.test(this.phone)){
                  ...
                  this.axios.post('/web/send/send',{mobile:this.phone,type:0}).then((res)=>{
            ...
            //验证身份证格式
            yancard(){
                var huzhaoreg=/^1[45][0-9]{7}$|([P|p|S|s]\d{7}$)|([S|s|G|g|E|e]\d{8}$)|([Gg|Tt|Ss|Ll|Qq|Dd|Aa|Ff]\d{8}$)|([H|h|M|m]\d{8,10})$/;//护照正则
                var reg=/^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/;//身份证正则
                $('#cardNum').css({display:'none'});
                if(this.zhengtype=='身份证'){
                    if(!reg.test(this.cardNum)){
                        $('#cardNum').css({display:'block'});
                    }
                }
                if(this.zhengtype=='护照'){
                    if(!huzhaoreg.test(this.cardNum)){
                        $('#cardNum').css({display:'block'});
                    }
                }
            },
